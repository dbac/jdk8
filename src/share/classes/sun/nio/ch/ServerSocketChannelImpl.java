    ...
    implements SelChImpl
{
    ...
    ServerSocketChannelImpl(SelectorProvider sp) throws IOException {
        super(sp);
        this.fd =  Net.serverSocket(true);  //创建一个文件描述符，FileDescriptor包装起来
        this.fdVal = IOUtil.fdVal(fd);  //获取真实的文件描述值
        this.state = ST_INUSE;   //设置状态值
    }
    ...
    public ServerSocket socket() {
        synchronized (stateLock) {  //同步状态锁
            if (socket == null)   //如果socket为空 创建socket
    ...
    public boolean translateReadyOps(int ops, int initialOps,
      ...
      // POLLIN = 1;// 文件描述符可读
        ...
        if (((ops & PollArrayWrapper.POLLIN) != 0) &&
                //SelectionKey.OP_ACCEPT   1<<4 =16     10000
