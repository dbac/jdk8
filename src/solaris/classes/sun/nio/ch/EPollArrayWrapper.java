...
class EPollArrayWrapper {
    ...
    EPollArrayWrapper() throws IOException {
        // 创建一个epoll socket（文件描述符）
        epfd = epollCreate();

        // the epoll_event array passed to epoll_wait
        // 计算大小
        //NUM_EPOLLEVENTS： 文件能打开的最大数目，最大不能超过8192
        //SIZE_EPOLLEVENT:  大数据大小
    ...
    void initInterrupt(int fd0, int fd1) {
        ...
        //把读通道文件描述符fd0注册到epft上（红黑树），关注EPOLLIN（连接到达；有数据来临）
    ...
    int poll(long timeout) throws IOException {
        ...
        //epollWait(long pollAddress, int numfds, long timeout,int epfd)
        //返回就绪的事件个数
        updated = epollWait(pollArrayAddress, NUM_EPOLLEVENTS, timeout, epfd);
        for (int i=0; i<updated; i++) {
              //判断是否是被中断了
            if (getDescriptor(i) == incomingInterruptFD) {  //如果是有数据来临的那个管道文件
                interruptedIndex = i;
                interrupted = true;
                break;
            }
